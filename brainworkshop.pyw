...
def parse_config(configpath):
    if CLINICAL_MODE and configpath == 'config.ini':
        ...
        if not os.path.isfile(os.path.join(get_data_dir(), configpath)):
            try:
                newconfigfile = open(os.path.join(os.path.join(get_data_dir(), configpath)), 'w')
                newconfigfile.write(CONFIGFILE_DEFAULT_CONTENTS)
                newconfigfile.close()
            except IOError: # read-only filesystem?  Complain without quitting
                quit_with_error(message='Could not write a new config file.  Is this a read-only filesystem?', 
                                quit=False)
                
        # The following is a routine to overwrite older config files with the new one.
        oldconfigfile = open(os.path.join(os.path.join(get_data_dir(), configpath)), 'r+')
        while oldconfigfile:
...
def set_user(newuser):
    ...
    if USER.lower() == 'default':
        ...
        try:
            ...
            f = file(os.path.join(get_data_dir(), cfg.STATSFILE), 'w')
            f.close()
        try:
